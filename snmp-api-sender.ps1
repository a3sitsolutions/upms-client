# Script SNMP para multiplas impressoras com envio para API
# Le printers-config.yml, consulta SNMP e envia dados para endpoint
...
function Send-PrinterDataToAPI {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "     Erro ao executar curl: $response" -ForegroundColor Red
            return $false
        }
    }
    catch {
...
function Read-PrintersConfigAndQuery {
    ...
    try {
        ...
        foreach ($printer in $config.printers) {
            ...
            # Envia dados para API se temos as informacoes necessarias
            if ($modelData -and $serialNumberData -and $totalPagesData -gt 0) {
                $apiSuccess = Send-PrinterDataToAPI -Model $modelData -SerialNumber $serialNumberData -TotalPrintedPages $totalPagesData
...
Write-Host "=== SNMP Multi-Printer Reader com API ===" -ForegroundColor Magenta
